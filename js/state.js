...
const PLAYER_WATER_HEIGHT = PLAYER_COLLISION_HEIGHT;
...
const SHADOW_MAN_SPAWN_UNLOCK_MINUTE = DEBUG_SHADOW_MAN_IGNORE_BUFFER ? 0 : FULL_CYCLE/1000/60; // 1 day and 1 night of peace
...
const SHADOW_MAN_PHASE3_SPAWN_THRESHOLD = 10;
const SHADOW_MAN_PHASE3_PLAYER_SPAWN_RADIUS = 500;
const SHADOW_MAN_CUTSCENE_TRIGGER_DIST = 67;
const SHADOW_MAN_CUTSCENE_MAX_ELEVATION_DELTA = 8;
const SHADOW_MAN_CUTSCENE_CENTER_NDC_X = 0.35;
const SHADOW_MAN_CUTSCENE_CENTER_NDC_Y = 0.55;
const SHADOW_MAN_CUTSCENE_SCREEN_EDGE_NDC = 0.95;
const SHADOW_MAN_CUTSCENE_STOP_DIST = 2;
const SHADOW_MAN_CUTSCENE_FREEZE_DURATION_SEC = 2.000;
const SHADOW_MAN_CUTSCENE_APPROACH_DURATION_SEC = 0.100;
const SHADOW_MAN_CUTSCENE_GAIT_CYCLE_HZ = 0; // was 5 but disabled gait
const SHADOW_MAN_CUTSCENE_GAIT_ARM_SWING = THREE.MathUtils.degToRad(0); // was 70 but disabled gait
const SHADOW_MAN_CUTSCENE_GAIT_LEG_SWING = THREE.MathUtils.degToRad(0); // was 60 but disabled gait
const SHADOW_MAN_CUTSCENE_RED_EYES_DURATION_SEC = 4.000;
const SHADOW_MAN_CUTSCENE_MIDNIGHT_PROGRESS = 19 / 24; // Midnight, with FULL_CYCLE starting at 5:00 AM
...
let creatureHouseRegions = []; // house interiors that creatures may enter only through open doors
let creatureCaveRegions = []; // cave interiors that creatures may enter only through the open side
let creatureCemeteryRegions = []; // cemetery interiors that regular creatures may enter only through open gates
let playerEnclosureRegions = []; // player-only regions with legal openings, used to prevent wall tunneling
let akChest = null;
let akChestGun = null;
...
const DRAGON_BOND_KILLS_REQUIRED  = 100; // dragon-beam kills needed to form bond
...
let hauntedHouseData = null;   // populated by createHauntedHouse()
let hhLastTorchState = null;   // tracks hasTorch across frames for item-lighting updates
let hhSeqPhase = 'none';       // 'none'|'active'|'ss_taken'|'hallway_exit'|'timer'|'angel_waves'|'flashbang'|'complete'
let hhSeqTimer = 0;            // seconds since stairs disappeared
let hhTorchExtinguished = false;
let hhSavedSequenceGameTime = -1; // gameTime to restore after the HH sequence
let hhFirstAngelSpawned = false;
let hhFirstAngelApproaching = false;
let hhAngels = [];             // HH sequence-only weeping angels
let hhAngelWaveStage = 0;      // index into HH post-first-angel wave schedule
let hhAngelWaveTimer = 0;      // seconds until next HH angel wave spawn
let hhAngelStageSpawnCount = 0;
